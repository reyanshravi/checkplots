...
export default function HotelUpdateForm({ hotelId, onButtonClick }) {
  ...
  useEffect(() => {
    if (hotelId) {
      axios
        .get(`http://localhost:7002/api/vendor/hotel/${hotelId}`)
        .then((response) => {
          setHotelData(response.data.hotel);
        })
        .catch((error) => {
          console.error("Error fetching hotel data:", error);
          setErrors({ form: "Error fetching hotel data." });
        });
    }
  }, [hotelId]);
  ...
  const validateForm = () => {
    ...
    if (!hotelData.name) errors.name = "Hotel name is required";
    if (!hotelData.type) errors.type = "Hotel type is required";
    if (!hotelData.details) errors.details = "Details are required";
    if (!hotelData.price) errors.price = "Price is required";
    if (!hotelData.pricePerNight)
      errors.pricePerNight = "Price per night is required";
    if (!hotelData.address) errors.address = "Address is required";
    if (!hotelData.contactNumber)
      errors.contactNumber = "Contact number is required";
    if (!hotelData.website) errors.website = "Website URL is required";
    if (!hotelData.availableRooms)
      errors.availableRooms = "Available rooms are required";
    ...
    if (hotelData.reviews < 0) errors.reviews = "Reviews cannot be negative";
  ...
  const imagePreview =
    hotelData.image &&
    Array.isArray(hotelData.image) &&
    hotelData.image.length > 0 ? (
      <div className="mt-2 flex space-x-2">
        {hotelData.image.map((imageUrl, index) => (
          <img
            key={index}
            src={imageUrl}
